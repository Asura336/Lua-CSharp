...
public struct Instruction : IEquatable<Instruction>
{
     ...
     * 
     * 
     * A 可能表示在栈（LuaStack）内寻找值的索引？ ScopeCompilationContext::StackPosition
     * B 是列表块的大小，最多 50；也会存储一个和 A 类似的值
     * C 被写入的变量叫 arrayBlock
     * 有一个测试用例（声明一个表，初始化长参数列表，参数数量 >100）会引发寻址异常，
     * 过多的参数让 A 的值溢出导致一个内部错误，在 LuaVirtualMachine::SetList(ref VirtualMachineExecutionContext)
     * 
     * 考虑扩增各个参数的范围
     * 另见 LuaCompiler::VisitTableConstructorExpressionNode(TableConstructorExpressionNode, ScopeCompilationContext)
     * 
     * Struct : u64
